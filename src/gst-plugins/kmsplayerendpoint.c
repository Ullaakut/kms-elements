...
#define PORT_RANGE_DEFAULT "60000-60200"
...
#define KMS_PLAYER_ENDPOINT_GET_PRIVATE(obj)                                   \
  (G_TYPE_INSTANCE_GET_PRIVATE((obj), KMS_TYPE_PLAYER_ENDPOINT,                \
...
G_DEFINE_TYPE_WITH_CODE(
    KmsPlayerEndpoint, kms_player_endpoint, KMS_TYPE_URI_ENDPOINT,
...
static void kms_pts_data_destroy(gpointer data) {
  g_slice_free(KmsPtsData, data);
}

static KmsPtsData *kms_pts_data_new() {
  KmsPtsData *data;
...
static void kms_pts_data_reset(KmsPtsData *data) {
  data->base_time = GST_CLOCK_TIME_NONE;
  data->last_pts_orig = GST_CLOCK_TIME_NONE;
}

static void kms_player_endpoint_set_caps(KmsPlayerEndpoint *self) {
  GstCaps *deco_caps;
...
void kms_player_endpoint_set_property(GObject *object, guint property_id,
...
void kms_player_endpoint_get_property(GObject *object, guint property_id,
  ...
  case PROP_VIDEO_DATA: {
    ...
    if (gst_element_query(playerendpoint->priv->pipeline, query)) {
      ...
      GST_WARNING_OBJECT(playerendpoint, "Impossible to get the file duration");
    }
    ...
    video_data = gst_structure_new(
        "video_data", "isSeekable", G_TYPE_BOOLEAN, seekable, "seekableInit",
        G_TYPE_INT64, segment_start, "seekableEnd", G_TYPE_INT64, segment_end,
...
static void kms_player_endpoint_disable_latency_probe(KmsPlayerEndpoint *self) {
  GstPad *pad;
...
/* This function must be called holding the element mutex */
static void kms_player_endpoint_enable_latency_probe(KmsPlayerEndpoint *self) {
  GstPad *pad;
...
static void kms_player_endpoint_dispose(GObject *object) {
...
static void kms_player_endpoint_finalize(GObject *object) {
  KmsPlayerEndpoint *self = KMS_PLAYER_ENDPOINT(object);
...
kms_player_endpoint_generate_base_time(KmsPlayerEndpoint *self) {
  ...
  base_time =
      gst_clock_get_time(clock) - gst_element_get_base_time(GST_ELEMENT(self));
  g_object_unref(clock);

  return base_time;
}

static GstClockTime kms_player_endpoint_get_or_generate_base_time(
  ...
  if (*base_time_in != GST_CLOCK_TIME_NONE) {
    ...
    *base_time_in = base_time;
    ...
    GST_DEBUG_OBJECT(self, "Setting base time to: %" GST_TIME_FORMAT
                           ", is preroll: %d",
...
static void kms_player_endpoint_reset_base_time(KmsPlayerEndpoint *self) {
  BASE_TIME_LOCK(self);
  if (self->priv->reset) {
...
static void kms_player_endpoint_mark_reset_base_time(KmsPlayerEndpoint *self) {
  BASE_TIME_LOCK(self);
...
kms_player_endpoint_mark_reset_base_time_and_set_state(KmsPlayerEndpoint *self,
                                                       GstState state) {
...
static GstFlowReturn process_sample(GstAppSink *appsink, GstAppSrc *appsrc,
                                    GstSample *sample, gboolean is_preroll) {
  ...
  pts_data = (KmsPtsData *)g_object_get_qdata(G_OBJECT(appsink), pts_quark());
  ...
  GST_LOG_OBJECT(appsink, "Is preroll: %d, buffer: %" GST_PTR_FORMAT
  ...
  if (pts_data->last_pts != GST_CLOCK_TIME_NONE &&
    ...
    GST_ERROR_OBJECT(
        appsink, "Non incremental PTS assignment (last PTS: %" GST_TIME_FORMAT
...
push:
  ...
  buffer = NULL;
  if (ret != GST_FLOW_OK) {
    GST_ERROR_OBJECT(appsink, "Could not send buffer to appsrc %s. Cause: %s",
...
static GstFlowReturn new_preroll_cb(GstAppSink *appsink, gpointer user_data) {
  GstSample *sample;
...
static GstPadProbeReturn main_pipeline_probe(GstPad *pad, GstPadProbeInfo *info,
                                             gpointer element) {
...
static GstElement *kms_player_end_point_add_appsrc(KmsPlayerEndpoint *self,
  ...
  g_object_set(G_OBJECT(appsrc), "is-live", TRUE, "do-timestamp", TRUE,
               ...
               G_GUINT64_CONSTANT(0), "format", GST_FORMAT_TIME, "emit-signals",
               FALSE, NULL);
...
static void kms_player_end_point_add_stat_probe(KmsPlayerEndpoint *self,
                                                GstPad *pad,
                                                KmsMediaType type) {
  KmsStatsProbe *probe;
...
static void kms_player_end_point_remove_stat_probe(KmsPlayerEndpoint *self,
                                                   GstPad *pad) {
  KMS_ELEMENT_LOCK(self);
...
static void pad_added(GstElement *element, GstPad *pad,
  ...
  if (agnosticbin != NULL) {
    ...
    g_object_set_qdata_full(G_OBJECT(appsink), pts_quark(), kms_pts_data_new(),
...
static void kms_remove_element_from_bin(GstBin *bin, GstElement *element) {
  GST_DEBUG("Removing %" GST_PTR_FORMAT " from %" GST_PTR_FORMAT, element, bin);
...
static void pad_removed(GstElement *element, GstPad *pad,
                        KmsPlayerEndpoint *self) {
  GstElement *appsink, *appsrc;
...
static gboolean kms_player_endpoint_stopped(KmsUriEndpoint *obj,
                                            GError **error) {
  ...
  KMS_URI_ENDPOINT_GET_CLASS(self)
      ->change_state(KMS_URI_ENDPOINT(self), KMS_URI_ENDPOINT_STATE_STOP);

  return TRUE;
}

static gboolean kms_player_endpoint_started(KmsUriEndpoint *obj,
                                            GError **error) {
  KmsPlayerEndpoint *self = KMS_PLAYER_ENDPOINT(obj);

  GST_DEBUG_OBJECT(self, "Pipeline started");

  /* Set source property in uridecodebin */
  g_object_set(G_OBJECT(self->priv->uridecodebin), "source", self->priv->rtspsrc, NULL);

  /* Set internal pipeline to playing */
  gst_element_set_state(self->priv->pipeline, GST_STATE_PLAYING);

  KMS_URI_ENDPOINT_GET_CLASS(self)
      ->change_state(KMS_URI_ENDPOINT(self), KMS_URI_ENDPOINT_STATE_START);

  return TRUE;
}

static gboolean kms_player_endpoint_set_position(KmsPlayerEndpoint *self,
                                                 gint64 position) {
  GstQuery *query;
  GstEvent *seek;
  gboolean seekable = FALSE;
...
static gboolean kms_player_endpoint_paused(KmsUriEndpoint *obj,
                                           GError **error) {
  KmsPlayerEndpoint *self = KMS_PLAYER_ENDPOINT(obj);
  ...
  if (ret == GST_STATE_CHANGE_SUCCESS) {
    ...
    gst_element_query_position(self->priv->pipeline, GST_FORMAT_TIME,
                               &position);
  ...
  KMS_URI_ENDPOINT_GET_CLASS(self)
      ->change_state(KMS_URI_ENDPOINT(self), KMS_URI_ENDPOINT_STATE_PAUSE);

  return TRUE;
}

static void configure_latency_probes(GstPad *pad, KmsStatsProbe *probe,
                                     gboolean *enabled) {
  if (*enabled) {
    kms_stats_probe_latency_meta_set_valid(probe, TRUE);
  } else {
...
static void kms_player_endpoint_update_media_stats(KmsPlayerEndpoint *self) {
...
static void kms_player_endpoint_collect_media_stats(KmsElement *obj,
                                                    gboolean enable) {
  KmsPlayerEndpoint *self = KMS_PLAYER_ENDPOINT(obj);
...
static void kms_player_endpoint_class_init(KmsPlayerEndpointClass *klass) {
  ...
  gst_element_class_set_static_metadata(
      GST_ELEMENT_CLASS(klass), "PlayerEndpoint", "Sink/Generic",
      "Kurento plugin player end point",
  ...
  g_object_class_install_property(
      gobject_class, PROP_USE_ENCODED_MEDIA,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_int64(
          "position", "position", "Playing position in the file as miliseconds",
          0, G_MAXINT64, 0, G_PARAM_READABLE | GST_PARAM_MUTABLE_READY));

  g_object_class_install_property(
      gobject_class, PROP_NETWORK_CACHE,
      g_param_spec_int("network-cache", "Network cache",
                       "When using rtsp sources, the amount of ms to buffer", 0,
                       G_MAXINT, NETWORK_CACHE_DEFAULT,
                       G_PARAM_READWRITE | GST_PARAM_MUTABLE_READY));

  g_object_class_install_property(
      gobject_class, PROP_PIPELINE,
      g_param_spec_object("pipeline", "pipeline", "Players private pipeline",
                          GST_TYPE_ELEMENT,
                          G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

  kms_player_endpoint_signals[SIGNAL_EOS] =
      g_signal_new("eos", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(KmsPlayerEndpointClass, eos_signal), NULL,
  ...
  kms_player_endpoint_signals[SIGNAL_INVALID_URI] =
      g_signal_new("invalid-uri", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(KmsPlayerEndpointClass, invalid_uri_signal),
  ...
  kms_player_endpoint_signals[SIGNAL_INVALID_MEDIA] = g_signal_new(
      ...
      G_STRUCT_OFFSET(KmsPlayerEndpointClass, invalid_media_signal), NULL, NULL,
...
static gboolean kms_player_endpoint_emit_EOS_signal(gpointer data) {
...
static gboolean kms_player_endpoint_emit_invalid_uri_signal(gpointer data) {
  KmsPlayerEndpoint *self = KMS_PLAYER_ENDPOINT(data);

  GST_DEBUG("Emit invalid uri signal");
  g_signal_emit(G_OBJECT(self), kms_player_endpoint_signals[SIGNAL_INVALID_URI],
                0);

  return G_SOURCE_REMOVE;
}

static gboolean kms_player_endpoint_emit_invalid_media_signal(gpointer data) {
  KmsPlayerEndpoint *self = KMS_PLAYER_ENDPOINT(data);
...
static ErrorData *create_error_data(KmsPlayerEndpoint *self,
                                    GstMessage *message) {
  ErrorData *data;

  data = g_slice_new(ErrorData);
  data->self = g_object_ref(self);
  data->message = gst_message_new_custom(
      GST_MESSAGE_TYPE(message), (GST_OBJECT(self)),
      gst_structure_copy(gst_message_get_structure(message)));

  return data;
}

static void delete_error_data(gpointer d) {
  ErrorData *data = d;
...
static gboolean kms_player_endpoint_post_media_error(gpointer d) {
  ErrorData *data = d;
...
static GstBusSyncReply bus_sync_signal_handler(GstBus *bus, GstMessage *msg,
  ...
  if (GST_MESSAGE_TYPE(msg) == GST_MESSAGE_EOS) {
    kms_loop_idle_add_full(self->priv->loop, G_PRIORITY_HIGH_IDLE,
                           kms_player_endpoint_emit_EOS_signal,
                           g_object_ref(self), g_object_unref);
  } else if (GST_MESSAGE_TYPE(msg) == GST_MESSAGE_ERROR) {

    if (g_str_has_prefix(GST_OBJECT_NAME(msg->src), "decodebin")) {
      kms_loop_idle_add_full(self->priv->loop, G_PRIORITY_HIGH_IDLE,
                             kms_player_endpoint_emit_invalid_media_signal,
                             g_object_ref(self), g_object_unref);
    } else if (g_strcmp0(GST_OBJECT_NAME(msg->src), "source") == 0) {
      kms_loop_idle_add_full(self->priv->loop, G_PRIORITY_HIGH_IDLE,
                             kms_player_endpoint_emit_invalid_uri_signal,
                             g_object_ref(self), g_object_unref);
    } else {
      ...
      kms_loop_idle_add_full(self->priv->loop, G_PRIORITY_HIGH_IDLE,
                             kms_player_endpoint_post_media_error, data,
                             delete_error_data);
    }
  }
  return GST_BUS_PASS;
...
static void source_setup_cb(GstElement *uridecodebin, GstElement *source,
                            KmsPlayerEndpoint *self) {
  GstPad *srcpad;
...
static void element_added(GstBin *bin, GstElement *element, gpointer data) {
  ...
  if (g_strcmp0(gst_plugin_feature_get_name(
                ...
                RTSPSRC) == 0) {
    g_object_set(G_OBJECT(element), "latency", self->priv->network_cache, NULL);
    g_object_set(G_OBJECT(element), "port_range", self->priv->port_range, NULL);
  }
}

static gboolean process_bus_message(GstBus *bus, GstMessage *message,
                                    gpointer data) {
  return TRUE;
}

static void kms_player_endpoint_init(KmsPlayerEndpoint *self) {
  GstBus *bus;
  ...
  /* Connect to signals */
  g_signal_connect(self->priv->uridecodebin, "pad-added", G_CALLBACK(pad_added),
                   self);
